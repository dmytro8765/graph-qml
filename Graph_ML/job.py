...
from src import circuit, performance, utils, performance_supervised, performance_supervised_max_cut, performance_subgraph
...
if __name__ == "__main__":
    ...
    parser.add_argument("-sub", "--subsize", help="Size of the subgraph to be found", default=4, type=int)
    parser.add_argument("-n", "--name", help="Number to add to filenames to prevent overrides", default=3, type=int)
    parser.add_argument("-b", "--base", help="Base directory", default="/home/Users/Quantum_Computing/Pennylane/Graph_ML", type=str)
    ...
    elif flags.circuit == "Sub_Sn_circuit":
        weight_shapes = {"weights_sn": (flags.layers, 4)}
        circ = circuit.subgraph_other_circuit
    ...
    base_output = pathlib.Path("/Users/home/Quantum_Computing/Pennylane/Graph_ML/output/subgraph")
    ...
    elif flags.task == 'Subgraph':

        dataset = utils.load_patterns_subgraph(base / flags.data, flags.qubits, flags.subsize)
        predictions, targets, weights = performance_subgraph.fit(qnode, weight_shapes, dataset, samplings=flags.samplings, epochs=flags.epochs)

        ext = f"Subgraph-{flags.circuit}-{flags.qubits}-{flags.subsize}-{n_parameters}-sampling_{flags.samplings}-epochs_{flags.epochs}-"
        pd.DataFrame(targets["train"], columns=["sampling", "target"]).explode("target")\
        .to_csv(base_output / (ext + f"targets-train-{flags.name}.csv"), index=False)
        print('First file saved')
        pd.DataFrame(predictions["train"], columns=["sampling", "epoch", "prediction"]).explode("prediction")\
