...
def hamilton(G):
    ...
    while F:
        ...
        neighbors = (node for node in graph.neighbors(path[-1]) 
                     if node not in path)  # Exclude self-loops and already visited nodes
...
for _ in range(num_samples):
    ...
    if(type_graph == 0):
        ...
        while hamilton(g) is None:
            p = random.uniform(0.3, 0.9)
            g = nx.gnp_random_graph(qubits, p)
        array = np.append(nx.to_numpy_array(g).flatten(), 1)
    else:
        g = nx.gnp_random_graph(qubits, p)
        while hamilton(g) is not None:
            p = random.uniform(0.3, 0.9)
            g = nx.gnp_random_graph(qubits, p)
...
torch.save(dataset, "/Users/home/qiskit_env/Pennylane/data/hamiltonian/nodes_8-graphs_3000.pt")
